...
# Assim como vimos na aula de hoje, solicito a criação de Scenarios~e Scenarios Outline para as questões a seguir.
...
Scenario outline: Verificação de classificação por pontos (Transição de Estado)
	...
	And está classificado como “<classificacao_inicial>”
...
4 -  [Heurísticas de Nielsen] Um sistema de upload de arquivos deve informar visualmente ao usuário quando um upload está em andamento, concluído ou falhou. Crie um caso de teste validando a aplicação da heurística de visibilidade do status do sistema durante o upload de arquivos grandes (>100MB).
...
Um sistema de atendimento via chat online é utilizado por operadores que ficam logados das 08h às 18h, atendendo múltiplos clientes durante o expediente. Crie um caso de teste baseado na heristica  "All-Nighter"
